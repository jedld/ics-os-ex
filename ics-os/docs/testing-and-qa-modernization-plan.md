...
| Host-native unit tests | Emerging | `test-io-unit` has 98 TAP cases covering cache/device lifecycle, completion retention, USB media identity, coherent/streaming/SG/bounce DMA ownership, IRQ domains, generic IOMMU requester/mapping/fault policy, and valid/malformed DMAR structures; `test-partition-unit` adds 15 TAP cases for the partition layer (IEEE CRC-32 vectors/chunking, ATA LBA28/LBA48 capacity decode); `test-klog-unit` adds 19 TAP cases for the kernel-log ring (init/empty, append, wrap-around eviction, text truncation, clear); `test-fatchain-unit` adds 10 TAP cases for the bounded FAT cluster-chain walk (valid step, EOC, corrupt next pointer, loop detection, degenerate maxent); `test-fbconsole-unit` adds 35 TAP cases for GOP/VBE pitch (packed and padded 1920x1200x32 accepted; undersize pitch rejected), late WC GOP map policy (below 4 GiB identity and above 4 GiB high-window both allowed; wrap/missing tag refused), 2MiB PDE PAT-WC encoding, 80x25 grid origin, and per-axis zoom (1920x1080 is 3x2); `test-xhcipolicy-unit` adds 15 TAP cases for xHCI MSI-X vs poll (no COM1 and x2APIC refuse MSI-X), port CCS bits, and control TD flags (Setup Chain is RsvdZ; IN Setup TRT=3); `test-consolemux-unit` adds 12 TAP cases for tmux status-line NUL termination (no stack garbage after `console(0)`) and scrollback view mapping; `test-cdcacm-unit` adds 16 TAP cases for CDC-ACM config parsing (QEMU usb-serial ACM, Pico IAD, QEMU FTDI vendor bulk serial, MSC/HID/truncated rejection); `test-usbdbg-unit` adds 16 TAP cases for the CDC debug RPC parser (KEYS hex, SCREEN dump, PPM size, KEXEC size); `test-ttycanon-unit` adds 8 TAP cases for canonical tty `read()` remainder (1-byte reads must not drop the rest of the line); `test-termcap-unit` adds 32 TAP cases for the shared SDK termcap module (`tgetent` buffer order, `tgetnum`/`tgetflag`/`tgetstr`, termcap escape decoding, caller-buffer advancement, `tgoto`/`tparam` expansion, `tputs` padding stripping, file loading, unknown terms, and null-buffer behavior); `test-pciscan-unit` adds 5 TAP cases for PCI slot function-count (empty 0xFFFF slots probe nothing); no general runner, fixture API, translating hardware-IOMMU backend, or coverage gate exists yet. |
...
| Hardware qualification | Missing | No board inventory, test agent, results service, or compatibility matrix is defined. The Pico 2 W Wi-Fi HTTP API is the first physical-debug agent surface (`GET /v1`); it is not yet a results service. |
...
|---|---|---|---|
| Boot and scheduler | `test-boot`, `test-smp`, `test-klog`, `test-usb-uefi`, `test-usb-uefi-gpt`, `test-ide-thumbdrive`, `test-screenshot`, `test-vbox-uefi-gpt`, `test-vbox-uefi-gpt-bios`, `test-bochs-uefi-gpt` | GRUB/Multiboot2 boot, root mount, AP startup, basic work stealing, no observed GPF; `test-klog` adds the build banner plus `version`/`uname`/`dmesg` console-observability gate (timestamped kernel-log records); `test-usb-uefi` boots the USB thumbdrive under OVMF from a USB mass-storage device and asserts UEFI firmware handoff, `usb0p0` root selection, and AP scheduling; `test-usb-uefi-gpt` boots the GPT ESP Etcher image (`ics-os-uefi.img`) under OVMF on q35 xHCI and asserts `BOOTX64.EFI` handoff, `GPT_DETECT usb0`, USB root, and GOP; `test-vbox-uefi-gpt` / `test-vbox-uefi-gpt-bios` boot that image in VirtualBox EFI and BIOS; `test-bochs-uefi-gpt` boots it under Bochs BIOS; `test-ide-thumbdrive` boots the thumbdrive image attached as an IDE/PATA disk and asserts the MBR parses (partition registered) and the FAT root mounts — regression for the signed-`char` MBR-magic comparison that rejected a valid `0x55 0xAA` signature; `test-screenshot` runs the in-OS framebuffer PPM capture and validates the host-readback file header, geometry, and exact byte count | Small fixed scenario; no topology, hotplug, preemption, lock, or long-run matrix |
| Process and ABI | `test-exec`, `test-spawn`, `test-vim`, `test-dup`, `test-termtest`, `test-htop` | ELF64 load/execute (`hello`, `spawn`, and the FEAT_TINY vim TUI via non-interactive `vim --version`), `posix_spawn()`, `waitpid()`, writable work disk, `dup(2)` (`0xC5`) runtime semantics, and terminal-stack behavior: canonical/raw `termios` round-trip, `TIOCGWINSZ`, monotonic clock, zero-timeout `select`/`poll`, DSR-6 cursor reporting, relative cursor motion, edge clamping, DECSC/DECRST, huge `SU` clamping, and OSC `ST` termination with embedded CSI on framebuffer and serial-backed ttys; `test-htop` exercises the new `sys_icsos_proc_list`, `sys_icsos_sysinfo`, and `sys_icsos_kill` ABI from a userland monitor with version/selftest/frame/dump modes | Smoke cases only; vim gate checks the version banner and clean exit, not editor behavior; spawn test does not assert decoded child status or abnormal exits; `test-dup` proves `dup()` allocates a distinct slot, that a dup'd tty fd is closable, and that a dup'd file fd shares the open description (write-through-duplicate visible after `fsync()`); serial DSR-6 uses a lightweight cursor model with basic motion and DECSC/DECRST support rather than a full serial VT emulation |
...
| GCC path | `test-cc1`, `test-gcc`, `test-gccdriver`, `test-dist`, `test-gcc-kbuild`, `test-kbuild` | Frontend, driver, toolchain, kernel generation, kexec, and capability checks; `test-dist` boots a BIOS+UEFI FAT thumb-drive distribution and drives cc1/as/ld through `gccdrv` against a long-named ldscript, then execs the result (`GCC_DRIVER_OK`/`DIST_GCC_OK`/`GCC_DRV_RUN_OK`) — regression for FAT LFN padding that corrupted VFS node names | KVM/host-CPU dependence for most jobs; long feedback cycle |
...
| Host unit tests | `test-io-unit`, `test-partition-unit`, `test-gpt-unit`, `test-klog-unit`, `test-fatchain-unit`, `test-smpclaim-unit`, `test-fbconsole-unit`, `test-bridge-console-unit`, `test-kbdleds-unit`, `test-lapicx2-unit`, `test-xhcipolicy-unit`, `test-consolemux-unit`, `test-cdcacm-unit`, `test-usbdbg-unit`, `test-ttycanon-unit`, `test-termcap-unit`, `test-pciscan-unit`, `test-net-unit` | Pure-logic verification off-target: I/O cache/device lifecycle and DMAR, IEEE CRC-32 and ATA LBA28/LBA48 decode, GPT detection/parsing/CRC, kernel-log ring semantics (init/append/wrap/truncate/clear), the bounded fail-closed FAT cluster-chain walk, GOP/VBE pitch acceptance (packed and padded Intel GOP rows), the ESP32 debug-bridge LCD console line buffer (commit/truncate/scroll/dirty tracking; compiles `extras/ics-esp32-serial-bridge/src/bridge_console.c` so the tested logic is the firmware logic), CDC debug RPC plus ICSOS_VER bind/STATUS stamps (`tests/usb_debug_unit.c`), Internet checksum / ARP / ICMP echo transform (`tests/net_*_unit.c`), shared SDK termcap parsing/expansion (`tests/termcap_unit.c`), and the SMP claim / `irq_kstack_dest` / leftover-current predicates (process kstack top only from the user stack; leftover USER on `pagedir1` keeps the USER crit token and must skip `vfs_busy`; leftover USER A on USER B is leftover-mm only; ready-ring `next`/`before` must be a kernel/kheap PCB (`sched_link_ok`); unclaimed ACCESS_SYS on idle BSS must repair; reserved idle slots are 64KiB (cert 248137 overflowed 32KiB); torn 64-bit slot detector — original causes of cert `PF64 rip=0x100000001000`, leftover `task_mgr` `UD64` into `cpus[]`, cert 248132 `CRIT-NONOWNER self=0x1`, cert 248133 `GPF64` in `scheduler`, and cert 248137 `IDLE-STACK-OVERFLOW`) | Host-only; no in-kernel or guest execution, no coverage gate |
