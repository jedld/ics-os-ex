...
typedef struct _process_mem {
    ...
    DWORD pages;                 //pages
    struct _process_mem *next;   //pointer to next
} process_mem;
...
//aka Task State Segment used for context switching
//used to save the state of a process
...
typedef struct _IPC_message {
   ...
   DWORD message, data;       //message is the message type
} IPC_message;
...
//the following functions are documented and defined in process.c
